...
class SessionManager:
    ...
    def _get_active_count(self) -> int:
        ...
        Filter by status == ACTIVE: end_session() keeps the key alive for 24h
        for analytics (see line 246), so a raw SCAN would count completed
        sessions against the system-wide cap. Per-key HGET is acceptable at
        pilot scale (max_concurrent_sessions=20 → ≤20 HGETs per create).
        """
        if self.redis:
            count = 0
            cursor = 0
            while True:
                ...
                for key in keys:
                    if self.redis.hget(key, "status") == SessionStatus.ACTIVE.value:
                        count += 1
                if cursor == 0:
                    break
            return count
